...
export class ParcelComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.parcelService
                ...
                .subscribe(
                    (res: HttpResponse<IParcel[]>) => this.paginateParcels(res.body, res.headers),
    ...
    registerChangeInParcels() {
        this.eventSubscriber = this.eventManager.subscribe('parcelListModification', response => this.loadAll());
    }
    ...
    protected paginateParcels(data: IParcel[], headers: HttpHeaders) {
